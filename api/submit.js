...
export default async function handler(req, res) {
  ...
  if (rowErr) {
    await recordAudit(supabase, {
      form: 'campaign-intake', client_id: clientId, stage: 'supabase_insert',
      outcome: 'error', detail: rowErr.message,
    });
  ...
  try {
    ...
    await recordAudit(supabase, {
      form: 'campaign-intake', client_id: clientId, stage: 'clickup_sync',
    ...
    await supabase.from('campaign_intakes').update({
  ...
  await syncSheets({ state, secrets, clientId, submittedAt, supabaseRowId: row.id })
    .then(() => recordAudit(supabase, {
      form: 'campaign-intake', client_id: clientId, stage: 'sheet_log',
